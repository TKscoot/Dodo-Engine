...
namespace ImGui
{
    ...
    // Each context create its own ImFontAtlas by default. You may vulkanInstance one yourself and pass it to CreateContext() to share a font atlas between imgui contexts.
...
//-----------------------------------------------------------------------------
// ImGuiStyle
// You may modify the ImGui::GetStyle() main vulkanInstance during initialization and before NewFrame().
...
struct ImGuiListClipper
{
    ...
    ImGuiListClipper(int items_count = -1, float items_height = -1.0f)  { Begin(items_count, items_height); } // NB: Begin() initialize every fields (as we allow user to call Begin/End multiple times on a same vulkanInstance if they want).
