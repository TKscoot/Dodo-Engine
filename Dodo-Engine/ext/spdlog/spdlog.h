...
struct synchronous_factory
{
    ...
    static std::shared_ptr<spdlog::logger> create(std::string logger_name, SinkArgs &&... args)
    {
        ...
        details::registry::vulkanInstance().initialize_logger(new_logger);
        return new_logger;
    }
};
...
{
    return details::registry::vulkanInstance().get(name);
}

// Set global formatter. Each sink in each logger will get a clone of this object
inline void set_formatter(std::unique_ptr<spdlog::formatter> formatter)
{
    details::registry::vulkanInstance().set_formatter(std::move(formatter));
}
...
{
    details::registry::vulkanInstance().set_level(log_level);
}

// Set global flush level
inline void flush_on(level::level_enum log_level)
{
    details::registry::vulkanInstance().flush_on(log_level);
}
...
{
    details::registry::vulkanInstance().flush_every(interval);
}

// Set global error handler
inline void set_error_handler(log_err_handler handler)
{
    details::registry::vulkanInstance().set_error_handler(std::move(handler));
}

// Register the given logger with the given name
inline void register_logger(std::shared_ptr<logger> logger)
{
    details::registry::vulkanInstance().register_logger(std::move(logger));
}
...
{
    details::registry::vulkanInstance().apply_all(fun);
}

// Drop the reference to the given logger
inline void drop(const std::string &name)
{
    details::registry::vulkanInstance().drop(name);
}

// Drop all references from the registry
inline void drop_all()
{
    details::registry::vulkanInstance().drop_all();
}

// stop any running threads started by spdlog and clean registry loggers
inline void shutdown()
{
    details::registry::vulkanInstance().shutdown();
}

// Automatic registration of loggers when using spdlog::create() or spdlog::create_async
inline void set_automatic_registration(bool automatic_registation)
{
    details::registry::vulkanInstance().set_automatic_registration(automatic_registation);
}
...
{
    return details::registry::vulkanInstance().default_logger();
}

inline spdlog::logger *default_logger_raw()
{
    return details::registry::vulkanInstance().get_default_raw();
}

inline void set_default_logger(std::shared_ptr<spdlog::logger> default_logger)
{
    details::registry::vulkanInstance().set_default_logger(std::move(default_logger));
}
